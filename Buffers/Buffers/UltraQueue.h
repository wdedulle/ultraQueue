...
BUFFERS_API unsigned int CALLING_CONVENTION BufferCreate (unsigned int size,unsigned int buffertype, unsigned int NrReadChannels, int * Status);
	//Create a new Buffer
	// Parameters :
	...
	//		- Function status code (User can check this if return value was 0 to know what happened)
	//			*  0 : No Error
	//			* -1 : A minimum of 1 ReadChannel must be requested
	//			* -2 : Requested buffertype is unknown
	//			* -3 : Out of memory
	// Return : 
	//		-  0 : Error while creating Buffer, see Status for details.
	//		- >0 : Buffer ID
...
BUFFERS_API unsigned int CALLING_CONVENTION BufferGetOverflow_Wait (unsigned int buffer_nr);
	//Flag to indicate :
